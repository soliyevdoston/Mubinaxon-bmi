...
import { LayoutDashboard, Users, BookOpen, BookMarked, Radio, Settings, LogOut, Sparkles } from 'lucide-react'
import { cn } from '@/lib/utils'

const navItems = [
  { href: '/admin/dashboard', label: 'Bosh sahifa',      icon: LayoutDashboard },
  { href: '/admin/users',     label: 'Foydalanuvchilar', icon: Users },
  { href: '/admin/subjects',  label: 'Fanlar',           icon: BookMarked },
  { href: '/admin/lessons',   label: 'Darslar',          icon: BookOpen },
  { href: '/admin/sessions',  label: 'Sessiyalar',       icon: Radio },
  { href: '/admin/settings',  label: 'Sozlamalar',       icon: Settings },
]

export function AdminSidebarNav({ user }: { user: { name?: string | null; email?: string | null } }) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 flex flex-col z-40"
      style={{ background: 'hsl(236 48% 6% / 0.95)', backdropFilter: 'blur(24px)', borderRight: '1px solid hsl(236 35% 14%)', boxShadow: '4px 0 24px hsl(250 85% 68% / 0.06)' }}>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}
              ...
              style={isActive ? { background: 'linear-gradient(135deg, hsl(250,85%,65%,0.2), hsl(280,75%,65%,0.12))', boxShadow: 'inset 0 0 0 1px hsl(250 85% 65% / 0.2)' } : undefined}>
              <div className={cn('w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-all', isActive ? 'opacity-100' : 'opacity-60 group-hover:opacity-100')}
                style={isActive ? { background: 'linear-gradient(135deg, hsl(250,85%,65%), hsl(280,75%,65%))' } : { background: 'hsl(236 42% 12%)' }}>
                <Icon className="w-3.5 h-3.5" style={isActive ? { color: 'white' } : {}} />
              </div>
              {item.label}
              {isActive && <div className="ml-auto w-1.5 h-1.5 rounded-full" style={{ background: 'linear-gradient(135deg, hsl(250,85%,70%), hsl(280,75%,70%))' }} />}
            </Link>
          )
        })}
      </nav>
