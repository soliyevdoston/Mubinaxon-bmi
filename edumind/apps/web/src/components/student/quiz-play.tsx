...
export function QuizPlay({ sessionId, userId }: Props) {
  ...
  async function handleAnswer(idx: number) {
    ...
    const res = await fetch(`/api/sessions/${sessionId}/answer`, {
      ...
      body: JSON.stringify({ questionId: currentQuestion.id, selectedIndex: idx, responseTimeMs }),
    })
    if (res.ok) {
      const data = await res.json()
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-4 relative overflow-hidden"
        ...
        <div className="relative z-10 w-full max-w-sm space-y-5">
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'hsl(236 48% 7%)', border: '1px solid hsl(236 35% 13%)' }}>
            ...
            {finalResults.slice(0, 5).map((r, i) => {
              ...
              return (
                <div key={r.userId} className="flex items-center gap-3 px-4 py-3 border-b last:border-0"
                  style={{ borderColor: 'hsl(236 35% 13%)', background: isMe ? 'hsl(37 90% 58% / 0.06)' : undefined }}>
                  ...
                  <span className="text-sm font-bold" style={{ color: isMe ? 'hsl(37,90%,65%)' : undefined }}>{r.totalScore}</span>
                </div>
              )
            })}
          </div>
          <button onClick={() => router.push('/student/home')}
            className="w-full h-12 rounded-xl text-sm font-semibold text-white flex items-center justify-center gap-2 transition-all hover:opacity-90 active:scale-[0.98]"
      ...
      <div className="min-h-screen flex items-center justify-center" style={{ background: 'hsl(var(--background))' }}>
        <div className="text-center space-y-3">
          <div className="w-12 h-12 rounded-2xl mx-auto flex items-center justify-center"
            style={{ border: '2px solid hsl(var(--primary))', borderTop: '2px solid transparent', animation: 'spin 1s linear infinite' }} />
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'hsl(var(--background))' }}>
      ...
      <div className="flex-1 flex flex-col px-4 py-5 max-w-lg mx-auto w-full space-y-5">
        <div className="rounded-2xl p-5" style={{ background: 'hsl(236 48% 7%)', border: '1px solid hsl(236 35% 13%)' }}>
        ...
        <div className="space-y-2.5">
          {currentQuestion.options.map((opt, idx) => {
            ...
            if (phase === 'answered' && shownCorrectIndex !== null) {
            ...
            return (
              <button key={idx} onClick={() => handleAnswer(idx)}
                ...
                className="w-full text-left rounded-xl border transition-all duration-200 disabled:pointer-events-none active:scale-[0.99]"
                style={{
                  padding: '14px 16px',
                  background: state === 'correct' ? 'hsl(155 60% 42% / 0.12)' : state === 'wrong' ? 'hsl(0 72% 62% / 0.1)' : state === 'selected' ? cfg.bg : state === 'dimmed' ? 'hsl(236 48% 6%)' : 'hsl(236 48% 8%)',
                  borderColor: state === 'correct' ? 'hsl(155 60% 42% / 0.5)' : state === 'wrong' ? 'hsl(0 72% 62% / 0.4)' : state === 'selected' ? cfg.border : state === 'dimmed' ? 'hsl(236 35% 12%)' : 'hsl(236 35% 14%)',
                  boxShadow: state === 'correct' ? '0 0 20px hsl(155 60% 42% / 0.2)' : state === 'wrong' ? '0 0 20px hsl(0 72% 62% / 0.15)' : state === 'selected' ? `0 0 20px ${cfg.glow}` : undefined,
        ...
        {phase === 'answered' && (
          <div className="rounded-xl p-4" style={{ background: 'hsl(236 48% 7%)', border: '1px solid hsl(236 35% 13%)' }}>
            ...
            {leaderboard.length > 0 && (
              <div className="space-y-1.5">
                ...
                {leaderboard.slice(0, 3).map((r, i) => {
                  const isMe = r.userId === userId
                  return (
                    <div key={r.userId} className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm ${isMe ? 'font-semibold' : ''}`}
