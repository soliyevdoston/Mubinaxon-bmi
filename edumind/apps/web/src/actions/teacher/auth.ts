...
export async function registerTeacher(formData: FormData): Promise<{ error?: string } | void> {
  ...
  await prisma.user.create({ data: { email, passwordHash, fullName, role: 'TEACHER' } })
}
