...
import { setDefaultTime, updateApiKey } from '@/actions/admin/settings'
...
export function SettingsClient({ defaultTime, savedKey }: { defaultTime: number; savedKey: string | null }) {
  ...
  function handleTimeClick(sec: number) {
    setActiveTime(sec)
    startTime(async () => { await setDefaultTime(sec) })
  }
    ...
    <div className="space-y-6">
      <div>
        <div className="flex gap-2">
          {[15, 30, 60].map((sec) => (
            <button key={sec} onClick={() => handleTimeClick(sec)} disabled={timePending}
              ...
              style={activeTime === sec ? {
                ...
                color: 'white', boxShadow: '0 4px 16px hsl(250 85% 65% / 0.35)',
              } : { background: 'hsl(236 42% 10%)', color: 'hsl(var(--muted-foreground))', border: '1px solid hsl(236 35% 14%)' }}>
      ...
      <div>
        ...
        <form onSubmit={handleKeySubmit} className="space-y-3">
          <div className="relative">
            <input name="apiKey" type={showKey ? 'text' : 'password'} defaultValue={savedKey ?? ''} placeholder="sk-ant-..."
              className="w-full h-11 rounded-xl px-4 pr-11 text-sm font-mono outline-none transition-all bg-[hsl(var(--input))] border border-[hsl(var(--border))] placeholder:text-[hsl(var(--muted-foreground))] focus:border-[hsl(var(--primary)/0.4)] focus:ring-2 focus:ring-[hsl(var(--primary)/0.15)]" />
          ...
          <button type="submit" disabled={keyPending}
            className="h-10 px-5 rounded-xl text-sm font-semibold text-white transition-all disabled:opacity-50 hover:opacity-90 active:scale-95 flex items-center gap-2"
            style={{ background: 'linear-gradient(135deg, hsl(37,90%,55%), hsl(20,85%,58%))', boxShadow: '0 4px 16px hsl(37 90% 55% / 0.3)' }}>
            {keyPending && <Loader2 className="w-3.5 h-3.5 animate-spin" />} Saqlash
          </button>
        </form>
      </div>
    </div>
