  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-64 flex flex-col z-40"
      style={{
        ...
        boxShadow: '4px 0 24px hsl(220 85% 65% / 0.06)',
      }}>
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link key={href} href={href}
              ...
              style={isActive ? {
                background: 'linear-gradient(135deg, hsl(220,85%,60%,0.2), hsl(250,80%,65%,0.12))',
                boxShadow: 'inset 0 0 0 1px hsl(220 85% 60% / 0.2)',
              } : undefined}>
              ...
              {isActive && (
                <div className="ml-auto w-1.5 h-1.5 rounded-full"
                  style={{ background: 'linear-gradient(135deg, hsl(220,85%,70%), hsl(250,80%,70%))' }} />
              )}
            </Link>
          )
        })}
